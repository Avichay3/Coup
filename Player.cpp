// Player.cpp – כולל Sanction + Baron::invest() + Spy::spyOn()
...
void Player::tax() {
    ...
    int amount = (role == Role::Governor) ? 3 : 2;
    coins += amount;
...
void Player::invest() {
    ...
    coins -= 3;
    coins += 6;
    std::cout << name << " invested 3 coins and received 6 coins (Baron)." << std::endl;
    endTurn();
}

void Player::spyOn(Player& target) {
    if (!alive || !target.isAlive()) throw std::logic_error("Both players must be alive.");
    if (role != Role::Spy) throw std::logic_error("Only Spy can use spyOn.");

    std::cout << name << " spies on " << target.getName() << ": they have " << target.getCoins() << " coins." << std::endl;
    game->blockArrest(&target);
    // לא מסיים תור – יכול לעשות פעולה נוספת (למשל gather)
}
